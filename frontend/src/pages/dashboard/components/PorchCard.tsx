...
function IconPencil({ className = "w-4 h-4" }: { className?: string }) {
  return (
    <svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
      <path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L6.832 19.82a4.5 4.5 0 0 1-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 0 1 1.13-1.897L16.863 4.487Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
    </svg>
  );
}
...
type Tab = "contact" | "music" | "details" | "comments" | "notes";
...
export default function PorchCard({
  ...
  const tabs: { id: Tab; label: string; icon: React.ReactNode }[] = [
    ...
    ...(onNotesChange
      ? [{ id: "notes" as Tab, label: "Admin Notes", icon: <IconPencil className="w-3.5 h-3.5" /> }]
      : []),
  ];
        ...
        return (
          <div className="text-sm text-gray-700">
            <p className="text-xs text-gray-400 mb-2">
              Internal notes — separate from the review process and not shown to applicants.
            </p>
            <textarea
              value={localAdminNotes}
              onChange={(e) => setLocalAdminNotes(e.target.value)}
              placeholder="Add internal notes about this porch..."
            ...
            <button
              ...
              onClick={async () => {
                ...
                try {
                  await onNotesChange(porch.id, localAdminNotes.trim() || null);
                } finally {
