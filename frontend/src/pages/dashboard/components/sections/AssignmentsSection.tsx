...
export default function AssignmentsSection({
  ...
  const allSelected = orgUsers.length > 0 && selectedUserIds.size === orgUsers.length;
  const someSelected = selectedUserIds.size > 0 && selectedUserIds.size < orgUsers.length;
  const unassignedCount = bands.filter((b) => b.assigned_reviewer_id == null).length;
        ...
        <p className="text-sm font-medium mb-4">
          {unassignedCount > 0 ? (
            <span className="text-porch-700">
              {unassignedCount} unassigned {unassignedCount === 1 ? "band" : "bands"} to distribute
            </span>
          ) : (
                      ...
                      {user.org_role && (
                        <span
                          className={`text-xs px-2 py-0.5 rounded-full border ${
                            ROLE_COLORS[user.org_role] || "bg-gray-100 text-gray-600 border-gray-200"
                          }`}
                        >
        ...
        <div className="mt-4 flex flex-col sm:flex-row items-start sm:items-center gap-4">
          ...
          >
            {assigning ? (
              ...
              <>Assign {unassignedCount} {unassignedCount === 1 ? "Band" : "Bands"} to {selectedUserIds.size} {selectedUserIds.size === 1 ? "Reviewer" : "Reviewers"}</>
            )}
          </button>
