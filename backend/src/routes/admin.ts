...
adminRouter.get('/bands', async (req, res) => {
  try {
    ...
    allBands.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())
...
adminRouter.get('/porches', async (req, res) => {
  try {
    ...
    allPorches.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())
...
adminRouter.get('/scheduling', async (req, res) => {
  try {
    const approvedBands = Array.from(bands.values()).filter(b => b.status === 'approved')
    const approvedPorches = Array.from(porches.values()).filter(p => p.status === 'approved')
